...
prefix = "nol_" # Prefix khusus untuk novel 0 mdpl
judul_novel = "0 mdpl"
...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {{
            ...
            localStorage.setItem('nol_last_read', currentFile);
            let riwayatBaca = JSON.parse(localStorage.getItem('nol_read_pages') || '[]');
            if (!riwayatBaca.includes(currentFile)) {{
                riwayatBaca.push(currentFile);
                localStorage.setItem('nol_read_pages', JSON.stringify(riwayatBaca));
            }}
        }});
    </script>
    <div class="container">
...
index_html += """        </ul>
    ...
    <script>
        function toggleTheme() { document.body.classList.toggle('light-mode'); }
        document.addEventListener("DOMContentLoaded", function() {
            let lastRead = localStorage.getItem('nol_last_read');
            if (lastRead) {
...
print(f"Selesai! {total_pages} halaman HTML berhasil digenerate untuk 0 mdpl.")
