...
prefix = "md_" # Prefix khusus untuk novel 3726 mdpl
judul_novel = "3726 mdpl"

if not os.path.exists(folder_output):
    os.makedirs(folder_output)

# Mengambil semua file gambar di dalam folder novel_gambar
list_gambar = sorted(glob.glob(f"{folder_gambar}/halaman_*.jpg")) # Ubah .jpg jadi .webp jika gambarmu formatnya webp

if not list_gambar:
    print("Gambar tidak ditemukan! Pastikan nama file halaman_001.jpg dst ada di folder novel_gambar.")
    exit()

pages = []
for path in list_gambar:
    nama_file_gambar = os.path.basename(path) # contoh: halaman_001.jpg
    # Mengambil angka dari nama file
    nomor = nama_file_gambar.replace("halaman_", "").replace(".jpg", "").replace(".webp", "")
    pages.append({"nomor": nomor, "file_gambar": nama_file_gambar})

total_pages = len(pages)

# --- 1. MEMBUAT HALAMAN BACA (HTML) ---
...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {{
            ...
            localStorage.setItem('mdpl_last_read', currentFile);
            let riwayatBaca = JSON.parse(localStorage.getItem('mdpl_read_pages') || '[]');
            if (!riwayatBaca.includes(currentFile)) {{
                riwayatBaca.push(currentFile);
                localStorage.setItem('mdpl_read_pages', JSON.stringify(riwayatBaca));
            }}
        }});
    </script>
    <div class="container">
...
# --- 2. MEMBUAT INDEX.HTML (DAFTAR ISI) OTOMATIS ---
...
# Memasukkan tombol 1 sampai 152 ke dalam daftar isi
for page in pages:
...
index_html += """        </ul>
    ...
    <script>
        function toggleTheme() { document.body.classList.toggle('light-mode'); }
        document.addEventListener("DOMContentLoaded", function() {
            let lastRead = localStorage.getItem('mdpl_last_read');
            if (lastRead) {
...
# Menyimpan file Daftar Isi (index.html)
with open("index.html", 'w', encoding='utf-8') as f:
    f.write(index_html)

print(f"Selesai! {total_pages} halaman HTML dan 1 file index.html (Daftar Isi) berhasil dibuat otomatis!")
